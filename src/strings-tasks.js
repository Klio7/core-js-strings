...
function removeLastOccurrences(str, value) {
  ...
  const str1 = str.slice(0, valueIndex);
  const str2 = str.slice(valueIndex + value.length);

  return valueIndex < 0 ? str : str1 + str2;
}
...
function countVowels(str) {
  ...
  const noVowelsStr = formattedStr.replaceAll(/[aieouy]/g, '');
  return formattedStr.length - noVowelsStr.length;
}
...
function isPalindrome(str) {
  const formattedStr = str.toLowerCase().replaceAll(/\W/g, '');
...
function invertCase(str) {
  ...
  const letters = wordArr.map((word) => word.split(''));
  const result = [];
  for (let i = 0; i < letters.length; i += 1) {
    result.push(
      letters[i].map((letter) =>
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (
      (arr[i].charCodeAt(0) < 78 && arr[i].charCodeAt(0) > 64) ||
      (arr[i].charCodeAt(0) < 110 && arr[i].charCodeAt(0) > 96)
    ) {
      result.push(String.fromCharCode(arr[i].charCodeAt(0) + 13));
    }
    if (
      (arr[i].charCodeAt(0) > 77 && arr[i].charCodeAt(0) < 91) ||
      (arr[i].charCodeAt(0) > 109 && arr[i].charCodeAt(0) < 123)
    ) {
      result.push(String.fromCharCode(arr[i].charCodeAt(0) - 13));
    }
